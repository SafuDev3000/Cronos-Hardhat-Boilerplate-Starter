...
import "contracts/crookRewards.sol";
contract crookNFTStaking is Ownable, IERC721Receiver {
  ...
  mapping(address => bool) unWhitelistedAddresses; 
 

   constructor(ERC721Enumerable _nft, crookRewards _token) { 
    nft = _nft;
    token = _token;
  }
...
modifier isWhitelisted(address _address) {
    ...
    function example(address _addressFromWhitelist) public view isWhitelisted(msg.sender) returns(bool){
    }
  ...
  function stake(uint256[] calldata tokenIds) external {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      require(whitelistedAddresses[msg.sender], "Whitelist: You need to be whitelisted and have a CELL to STAKE!");
